...
[AARM](https://aarm.dev) (Autonomous Action Runtime Management) is an open specification — a Cloud Security Alliance Powered Project authored by Herman Errico — that defines a system category for securing AI-agent actions at runtime: intercept an action, evaluate it against policy, decide on it, and **record a tamper-evident receipt** of what happened.
...
| The decision (R5) | `action.authorization.decision` — `allowed` / `denied` / `human_approved` (AARM R4 defines five decision values — ALLOW, DENY, MODIFY, STEP_UP, DEFER; this enum covers a subset) |
...
- **A witness layer.** AARM receipts are produced and held by the operator's own runtime system. That proves integrity, never completeness — an operator can simply decline to write the inconvenient record. halo-record ships a witness protocol: a party outside the operator holds a periodic count and head-hash of the chain, so a customer can verify *completeness* rather than take the operator's word. The protocol ships here; a hosted, recognized witness is still being built, and a witness the operator runs itself does not provide independence. The receipt requirements do not define this layer; it is where independence will come from. See [Integrity vs. completeness](README.md#integrity-vs-completeness-read-this-part).
